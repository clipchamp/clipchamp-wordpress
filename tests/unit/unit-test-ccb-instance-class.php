...
require_once(dirname(dirname(__DIR__)) . '/classes/ccb-instance-class.php');
 ...
 */
if ( ! class_exists('UnitTestCCB_Instance_Class') ) {
	class UnitTestCCB_Instance_Class extends UnitTestCase {
		/*
		 * validate_is_valid()
		 */
		public function test_is_valid() {
			...
			foreach ( $valid_data_set as $pair ) {
				try {
					$valid = new CCB_Instance_Class( $pair[0], $pair[1] );
					$this->pass();
			...
			foreach ( $invalid_data_set as $pair ) {
				try {
					$invalid = new CCB_Instance_Class( $pair[0], $pair[1] );
